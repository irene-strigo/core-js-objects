...
function sellTickets(queue) {
  if (queue.length === 1 && queue[0] === 25) {
    return true;
  }
  const reserve = queue.slice(0, -1).reduce((a, b) => a + b, 0);
  const last = queue.slice(-1);
  return reserve >= last;
}
...
function sortCitiesArray(arr) {
  const stringArr = [];
  arr.forEach((obj) => stringArr.push(JSON.stringify(obj)));
  const sorted = stringArr.sort();
  const res = [];
  sorted.map((str) => res.push(JSON.parse(str)));
  return res;
}
...
function group(array, keySelector, valueSelector) {
  ...
  for (let i = 0; i < array.length; i += 1) {
    ...
    key = keySelector(obj);
    if (multiMap.has(key)) {
      multiMap.get(key).push(valueSelector(obj));
    } else {
      multiMap.set(key, [valueSelector(obj)]);
    }
  }
  return multiMap;
}
